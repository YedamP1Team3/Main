...
const processReservation = async (rsvId, decision, rejectReason) => {
  try {
    ...
    const finalRejectReason =
      decision === "REJECTED" ? rejectReason.trim() : null;

    const result = await rsvMapper.updateReservationStatus(
      rsvId,
      decision,
      finalRejectReason,
    );
...
const completeReservation = async (rsvId) => {
  try {
    const result = await rsvMapper.updateReservationToCompleted(rsvId);

    if (!result || result.affectedRows === 0) {
      const error = new Error(
        "완료 처리할 수 없는 예약입니다. 승인 상태의 예약만 완료 처리할 수 있습니다.",
      );
      error.statusCode = 400;
      throw error;
    }
