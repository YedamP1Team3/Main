...
const fetchRejectHistory = async () => {
    ...
    try {
        const res = await axios.get(`/api/abc/admin/priority/reject-history/${targetBeneId.value}`);
        rejectHistoryList.value = res.data?.success ? res.data.data || [] : [];
    } catch (error) {
...
<template>
    <div v-if="isAdmin" class="priority-manage-container">
        ...
        <div v-if="currentStatus === 'pending'" class="action-area">
            ...
            <!-- 같은 대상자에 대해 예전 반려 사유를 바로 참고할 수 있도록 유지한 영역 -->
