...
<template>
    <div class="flex flex-col h-[40rem] bg-white shadow-md rounded-xl border-2 border-[#f4e2de] p-5">
            ...
            <ul v-if="subItems?.length" class="space-y-2">
                ...
                >
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <span v-if="selectedId === sub.id" class="text-[#ffab91] text-xs">●</span>
                        <button v-if="isActive" @click.stop="emit('delete-sub-item', sub.id)" class="text-slate-400 hover:text-[#ff8a65] font-bold px-1 transition-colors">✕</button>
                    </div>
                </li>
            </ul>
        ...
        <div v-if="isActive" class="pt-4 mt-4 border-t-2 border-[#f4e2de] shrink-0 flex gap-2">
            <input v-model="newSubItemName" type="text" @keyup.enter="handleAddSub" class="flex-1 bg-white border-2 border-[#f4e2de] rounded-lg px-3 py-2 text-sm outline-none focus:border-[#ffab91]" placeholder="새 세부항목" />
            <button @click="handleAddSub" class="bg-[#ffab91] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#ff8a65] transition-colors shadow-sm">추가</button>
        </div>
    </div>
</template>
<style scoped>
