...
const emit = defineEmits(['select', 'add-sub-item']);
        ...
        <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            <ul v-if="subItems?.length" class="space-y-2">
                <li
                    ...
                    :class="['p-3 rounded-lg cursor-pointer transition-all border font-medium flex justify-between', selectedId === sub.id ? 'bg-emerald-50 border-emerald-400 text-emerald-700' : 'bg-slate-50 hover:bg-slate-100']"
                >
                    <span>{{ sub.name }}</span>
