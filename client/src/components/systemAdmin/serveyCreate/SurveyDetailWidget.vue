    ...
    <div class="card bg-white shadow-xl rounded-2xl h-[40rem] flex flex-col border-2 border-[#f4e2de] overflow-hidden">
        <div class="bg-white border-b-2 border-[#f4e2de] p-5 shrink-0">
            <div class="flex items-center justify-between mb-3">
                ...
                <div v-if="isActive" class="flex gap-4 items-center">
                    <button @click="toggleAll" class="text-sm font-bold text-slate-400 hover:text-[#ffab91] transition-colors">질문 전체선택</button>
                    <button @click="handleDelete" class="px-3 py-1.5 bg-white border-2 border-[#f4e2de] text-[#ffab91] rounded text-xs font-bold hover:border-[#ff8a65] hover:text-[#ff8a65] hover:bg-[#fef9f6] transition-colors">선택질문 삭제</button>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-5 custom-scrollbar bg-white">
            ...
            <div v-if="showInput" class="mt-4 p-4 border-2 border-[#ffab91] bg-white rounded-xl shadow-sm">
                <textarea v-model="newTitle" placeholder="질문 입력..." class="w-full bg-transparent border-none outline-none focus:ring-0 text-sm font-semibold resize-none"></textarea>
                <div class="flex justify-end gap-2 mt-2">
                    <button @click="showInput = false" class="text-xs font-bold text-slate-400 hover:text-[#ff8a65] transition-colors">취소</button>
                    <button @click="handleSubmit" class="bg-[#ffab91] text-white px-4 py-1.5 rounded text-xs font-bold hover:bg-[#ff8a65] transition-colors shadow-sm">저장</button>
                </div>
            </div>
        </div>

        <div v-if="isActive && !showInput" class="p-4 bg-white border-t-2 border-[#f4e2de] shrink-0">
            <button @click="showInput = true" class="w-full py-3 border-2 border-dashed border-[#f4e2de] rounded-xl text-[#ffab91] font-bold hover:border-[#ffab91] hover:bg-[#fef9f6] transition-colors">+ 새 질문 추가하기</button>
        </div>
    </div>
</template>
<style scoped>
